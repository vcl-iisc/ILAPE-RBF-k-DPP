...
class Buffer:
    ...
    def __init__(self, buffer_size, device, n_tasks=None, mode="reservoir"):
        ...
        self.attributes = ["examples", "heatmaps", "labels", "logits", "task_labels"]
        self.images = None
        self.heatmaps = None
    ...
    def init_tensors(
        ...
        """
        self.examples = my_array = np.empty([self.buffer_size,], dtype=object)
        self.heatmaps = np.zeros(self.buffer_size,)
    ...
    def get_data(self, size: int, transform: transforms = None) -> Tuple:
        ...
        # ret_tuple = (
        #     torch.stack([transform(ee) for ee in self.examples[choice]]),
        # )
    ...
    def get_all_data(self, transform: transforms = None) -> Tuple:
        ...
        if transform is None:
            transform = lambda x: x
        ret_tuple = (torch.stack([transform(ee) for ee in self.examples]),)
        for attr_str in self.attributes[1:]:
            if hasattr(self, attr_str):
                attr = getattr(self, attr_str)
                ret_tuple += (attr,)
